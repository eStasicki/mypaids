...
export function generatePDFReport(months: Month[], title: string = 'Raport Rachunków'): void {
				...
				const tableData = [
					...
					...month.bills.map((bill, billIndex) => {
						...
						return [
							...
							{ text: amount, alignment: 'right', fillColor: billIndex % 2 === 0 ? '#ffffff' : '#f9fafb' },
							{ text: categoryName, fillColor: billIndex % 2 === 0 ? '#ffffff' : '#f9fafb' },
							{ text: comment, style: 'comment', fillColor: billIndex % 2 === 0 ? '#ffffff' : '#f9fafb' }
						];
					})
				];
	...
	pdfMake.createPdf(docDefinition).download(`raport-rachunkow-${new Date().toISOString().split('T')[0]}.pdf`);
}
