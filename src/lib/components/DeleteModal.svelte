<script lang="ts">
	let { show, title, message, onConfirm, onCancel }: {
		show: boolean;
		title: string;
		message: string;
		onConfirm: () => void;
...
{#if show}
	...
	>
		<div
			class="bg-gradient-to-br from-gray-800/95 to-gray-800/90 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-gray-700/50 max-w-md w-full mx-4 transform transition-all animate-fade-in"
		>
			<div class="flex items-start gap-4 mb-4">
				<div class="flex-shrink-0 w-12 h-12 rounded-xl bg-red-500/20 flex items-center justify-center border border-red-500/30">
					<svg
