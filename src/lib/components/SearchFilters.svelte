<script lang="ts">
	...
	let { filters, onFiltersChange }: { filters: SearchFilters; onFiltersChange: (filters: SearchFilters) => void } = $props();
	...
	<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
		...
		<div class="flex gap-2">
			...
			>
				...
				<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform {isExpanded ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
	...
	{#if isExpanded}
		<div class="space-y-4 pt-4 border-t border-gray-700/50">
			<div>
				...
				<div class="flex flex-wrap gap-2">
					{#each DEFAULT_CATEGORIES as category}
						<label
							class="flex items-center gap-2 px-3 py-2 rounded-lg border transition-all duration-200 cursor-pointer {filters.categoryIds.has(category.id)
								? 'bg-blue-600/20 border-blue-500/50'
								: 'bg-gray-700/30 border-gray-600/50 hover:bg-gray-700/50'}"
							style={filters.categoryIds.has(category.id) ? `border-left-color: ${category.color};` : ''}
						>
							<input
								type="checkbox"
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div>
					<label for="min-amount" class="block text-sm font-medium text-gray-300 mb-2">Minimalna kwota (zł)</label>
					<input
						id="min-amount"
						type="number"
						bind:value={filters.minAmount}
						oninput={(e) => updateFilters({ minAmount: e.currentTarget.value ? parseFloat(e.currentTarget.value) : null })}
						placeholder="0"
						min="0"
						step="0.01"
				...
				<div>
					<label for="max-amount" class="block text-sm font-medium text-gray-300 mb-2">Maksymalna kwota (zł)</label>
					<input
						id="max-amount"
						type="number"
						bind:value={filters.maxAmount}
						oninput={(e) => updateFilters({ maxAmount: e.currentTarget.value ? parseFloat(e.currentTarget.value) : null })}
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div>
					<label for="sort-by" class="block text-sm font-medium text-gray-300 mb-2">Sortuj według</label>
					<select
						id="sort-by"
						bind:value={filters.sortBy}
						onchange={(e) => updateFilters({ sortBy: e.currentTarget.value as 'date' | 'amount' | 'name' })}
				...
				<div>
					<label for="sort-order" class="block text-sm font-medium text-gray-300 mb-2">Kolejność</label>
					<select
						id="sort-order"
						bind:value={filters.sortOrder}
						onchange={(e) => updateFilters({ sortOrder: e.currentTarget.value as 'asc' | 'desc' })}
