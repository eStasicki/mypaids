  ...
  <table class="w-full" role="table">
    ...
    <tbody>
      {#each month.bills as bill (bill.id)}
        <tr
          class="border-b border-gray-700/30 hover:bg-gray-700/20 transition-all duration-150"
        >
          <td class="py-4 px-4" style="position: relative; overflow: visible;">
            {#if editingId === bill.id}
              <CategorySelector
                ...
                onSelect={(categoryId) =>
                  updateBill(bill.id, "categoryId", categoryId)}
              />
            {:else}
          ...
          <td class="py-4 px-4">
            {#if editingId === bill.id}
              <input
                ...
                oninput={(e) =>
                  updateBill(bill.id, "name", e.currentTarget.value)}
            ...
            {:else}
              <div>
                ...
                {#if bill.comment}
                  <div
                    class="text-xs text-gray-500 mt-1 italic flex items-start gap-1"
                  >
                    <span>💬</span>
          ...
          <td class="py-4 px-4 text-right">
            {#if editingId === bill.id}
              <input
                ...
                onkeydown={(e) => handleKeydown(e, bill.id)}
                oninput={(e) =>
                  updateBill(bill.id, "amount", e.currentTarget.value)}
                placeholder="-"
            ...
            <td colspan="4" class="py-3 px-4">
              <div>
                ...
                <textarea
                  value={bill.comment || ""}
                  oninput={(e) =>
                    updateBill(bill.id, "comment", e.currentTarget.value)}
      ...
      {/each}
      <tr
        class="bg-gradient-to-r from-gray-700/20 to-gray-700/10 border-t border-gray-700/30"
      >
        <td class="py-4 px-4" style="position: relative; overflow: visible;">
          <CategorySelector
