<script lang="ts">
	...
	function handleAddTemplate() {
		...
		const amount =
			templateAmount.trim() === "" || templateAmount.trim() === "-"
				? null
...
{#if show}
	<div
		...
		class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4 overflow-y-auto"
	>
		<div
			class="bg-linear-to-br from-gray-800/95 to-gray-800/90 backdrop-blur-sm rounded-2xl p-8 shadow-2xl border border-gray-700/50 max-w-4xl w-full my-auto max-h-[90vh] overflow-y-auto"
		>
			...
			{#if isAdding}
				<div class="mb-6 p-4 bg-gray-700/30 rounded-xl border border-gray-600/50">
					...
					<div class="space-y-4">
						<div>
							<label for="template-name" class="block text-sm font-medium text-gray-300 mb-2"
								>Nazwa</label
							>
							<input
								id="template-name"
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div>
								<label for="template-amount" class="block text-sm font-medium text-gray-300 mb-2"
									>Kwota (zł)</label
								>
								<input
									id="template-amount"
				...
				{#if $templates.length === 0}
					<div class="text-center py-12">
						...
						<p class="text-sm text-gray-500">
							Dodaj szablon, aby szybko dodawać powtarzalne rachunki
						</p>
					</div>
				{:else}
					{#each $templates as template (template.id)}
						<div
							class="bg-gray-700/30 rounded-xl p-4 border border-gray-600/50 hover:border-gray-600 transition-all"
						>
							<div class="flex items-start justify-between gap-4">
								<div class="flex-1">
									<div class="flex items-center gap-3 mb-2">
										...
										{#if template.autoAdd}
											<span
												class="px-2 py-1 text-xs bg-green-500/20 text-green-400 rounded border border-green-500/30"
											>
												Auto
											</span>
										{/if}
									...
									<div class="flex items-center gap-4 text-sm text-gray-400">
										<span
											>Kwota: {template.amount !== null
												? `${template.amount.toFixed(2)} zł`
												: "Brak"}</span
										>
										{#if template.categoryId}
											{#await loadCategory(template.categoryId) then category}
