<script lang="ts">
	...
	function saveDate() {
		...
		months.update((ms) => {
			const updated = ms.map((m) => (m.id === month.id ? { ...m, date: newDate } : m));
			return sortMonthsByDate(updated);
		});
		editingDate = false;
	}
	...
	function saveNotes() {
		months.update((ms) =>
			ms.map((m) => (m.id === month.id ? { ...m, notes: editNotes.trim() || undefined } : m))
		);
		editingNotes = false;
	}
...
<div
	class="group relative bg-linear-to-br from-gray-800/90 to-gray-800/70 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-gray-700/50 hover:border-gray-600/50 transition-all duration-300 hover:shadow-3xl hover:shadow-blue-500/10 hover:-translate-y-1 {isNewCard
		? 'animate-pulse-new-card'
		: ''}"
>
	<div
		class="absolute inset-0 bg-linear-to-br from-blue-500/0 to-purple-500/0 group-hover:from-blue-500/5 group-hover:to-purple-500/5 rounded-2xl transition-all duration-300 pointer-events-none"
	></div>
	...
	<div class="relative">
		...
		<div class="mt-6 pt-6 border-t border-gray-700/50">
			<div
				class="flex justify-between items-center bg-gradient-to-r from-gray-800/50 to-gray-800/30 rounded-xl p-4 border border-gray-700/30"
			>
				...
				>
					{getTotalForMonth(month).toFixed(2)}
					<span class="text-xl text-gray-400">zł</span>
				</span>
			</div>
		</div>
	</div>
...
<DeleteModal
	...
	title="Usuń miesiąc"
	message="Czy na pewno chcesz usunąć miesiąc {getMonthName(
		month.date
	)} {month.date.getFullYear()}? Ta operacja jest nieodwracalna."
