	...
	function getFilteredMonths() {
		...
		if (selectedCategories.size > 0) {
			filtered = filtered.map((month) => ({
				...month,
				bills: month.bills.filter((bill) => 
					!bill.categoryId || selectedCategories.has(bill.categoryId)
				)
			}));
		}
	...
	<header role="banner" class="mb-10">
			...
			<div class="bg-gradient-to-br from-gray-800/90 to-gray-800/70 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50 mb-6">
				<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
					<div>
						...
						<p class="text-sm text-gray-400">Wybierz kategorie, które mają być uwzględnione w wykresie</p>
					</div>
					<div class="flex gap-2">
						<button
				...
				<div class="flex flex-wrap gap-3">
					{#each getAvailableCategoryIds() as categoryId}
						...
						{#if category}
							<label
								class="flex items-center gap-2 px-4 py-2 rounded-lg border transition-all duration-200 cursor-pointer {selectedCategories.has(categoryId)
									? 'bg-blue-600/20 border-blue-500/50'
									: 'bg-gray-700/30 border-gray-600/50 hover:bg-gray-700/50'}"
								style={selectedCategories.has(categoryId) ? `border-left-color: ${category.color};` : ''}
							>
								<input
									type="checkbox"
		...
		<div class="flex flex-wrap gap-2 mb-6">
			...
			<button
				...
				aria-label={showDetailedStats ? 'Ukryj szczegółowe statystyki' : 'Pokaż szczegółowe statystyki'}
	...
	<div class="bg-gradient-to-br from-gray-800/90 to-gray-800/70 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-gray-700/50">
		...
		{:else if getFilteredMonths().length === 0}
			<div class="text-center py-16">
				...
				<p class="text-sm text-gray-500">Zaznacz przynajmniej jeden rok i kategorię, aby zobaczyć wykres</p>
			</div>
		{:else}
			<div class="mb-6">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
					{#each getCategoryTotals(getFilteredMonths()) as { category, total }}
						<div class="bg-gray-700/30 rounded-lg p-4 border border-gray-600/50" style="border-left: 4px solid {category.color};">
							<div class="flex items-center gap-2 mb-2">
								<span class="text-xl">{category.icon}</span>
								<span class="font-medium text-white">{category.name}</span>
							</div>
							<div class="text-2xl font-bold text-white">{total.toFixed(2)} zł</div>
						</div>
					{/each}
				</div>
			</div>
				...
				<div class="bg-gradient-to-br from-gray-800/90 to-gray-800/70 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-gray-700/50">
					...
					<div class="space-y-2">
						{#each mostExpensive as item}
							<div class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg border border-gray-600/50">
								<div class="flex items-center gap-3">
									...
									<div>
										...
										<div class="text-xs text-gray-400">
											{item.month.date.toLocaleDateString('pl-PL', { month: 'long', year: 'numeric' })}
										</div>
									</div>
								</div>
								<div class="text-right">
									<div class="font-bold text-white text-lg">{item.bill.amount?.toFixed(2)} zł</div>
									<div class="text-xs text-gray-400">z {item.totalInMonth.toFixed(2)} zł</div>
								</div>
							</div>
						{/each}
					</div>
				...
				<div class="bg-gradient-to-br from-gray-800/90 to-gray-800/70 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-gray-700/50">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						{#each categoryStats as stat}
							...
							{#if category}
								<div class="p-4 bg-gray-700/30 rounded-lg border border-gray-600/50" style="border-left: 4px solid {category.color};">
									<div class="flex items-center gap-2 mb-2">
							...
							<div class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg border border-gray-600/50">
								<div>
									<div class="font-medium text-white">
										{trend.month.toLocaleDateString('pl-PL', { month: 'long', year: 'numeric' })}
									</div>
								...
								<div class="text-right">
									<div class="font-bold text-white">{trend.total.toFixed(2)} zł</div>
									<div class="text-sm {trend.change > 0 ? 'text-red-400' : trend.change < 0 ? 'text-green-400' : 'text-gray-400'}">
										{trend.change > 0 ? '+' : ''}{trend.change.toFixed(2)} zł
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{#each yearComparison as comparison}
							<div class="p-4 bg-gray-700/30 rounded-lg border border-gray-600/50">
								<h4 class="text-lg font-semibold text-white mb-3">{comparison.year}</h4>
