...
if (!supabaseUrl) {
  console.error('Missing PUBLIC_SUPABASE_URL in .env');
  process.exit(1);
}

if (!supabaseServiceKey) {
  console.error('Missing SUPABASE_SERVICE_ROLE_KEY in .env');
  console.error('Get it from: https://supabase.com/dashboard/project/jrrylqtfsegsqzmckwdf/settings/api');
  process.exit(1);
}
...
async function seedDemoData() {
  try {
    const response = await fetch(`${supabaseUrl}/rest/v1/rpc/get_user_by_email`, {
      ...
      body: JSON.stringify({ user_email: 'estasicki@dev.com' })
    });
    ...
    if (!response.ok) {
      ...
      userId = profiles.id;
      console.log(`Found user: ${profiles.email} (${userId})`);
    } else {
      const userData = await response.json();
      userId = userData.id;
      console.log(`Found user: estasicki@dev.com (${userId})`);
    }
    ...
    for (const month of months) {
      const { data, error } = await supabase
        .from('months')
        .upsert({ user_id: userId, date: month.date, notes: month.notes }, { onConflict: 'user_id,date' })
        .select()
        .single();
      ...
      console.log(`✓ Created month: ${month.date} (${data.id})`);
    }
    ...
    for (const bill of bills) {
      ...
      inserted++;
      console.log(`✓ Inserted bill: ${bill.name} (${bill.amount} zł)`);
    }
